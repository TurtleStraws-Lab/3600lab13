...
int main(int argc, char *argv[])
{
	 ...
	 *
	 * The c
     * ontents of a file mapping are initialized using length bytes
	...
	if (addr == MAP_FAILED)
		perror("mmap");
	char *dest = mmap(NULL, sb.st_size , PROT_WRITE, MAP_SHARED 
            | MAP_ANONYMOUS, -1, 0);
    memcpy(dest, addr, length + offset - pa_offset);
